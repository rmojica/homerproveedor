...
export class ProductPage {
    ...
    constructor(public nav: NavController, public service: ProductService, params: NavParams, public functions: Functions, public values: Values) {
        ...
        if(params.data.id){
            ...
            this.usedVariationAttributes = this.product.product.attributes.filter(function (attribute) { return attribute.variation == true });
        }
        else {
    ...
    onDaySelect($event,id){
        ...
        if (this.values.isLoggedIn) {
            this.service.getBlocks(this.day, this.month, this.year, id).then((results) => this.update_blocks(results));
        } else {
            ...
            this.usedVariationAttributes.forEach(attribute => {
                if(variation.attributes.length == 0){
                    ...
                    variation.attributes.forEach((item, index) => {
                        if(attribute.selected && item.name.toUpperCase() == attribute.name.toUpperCase() && item.option.toUpperCase() == attribute.selected.toUpperCase()){
                            test[index] = true;
                        }
                    })
